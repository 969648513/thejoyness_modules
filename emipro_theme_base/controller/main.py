...
class CustomerPortal(CustomerPortal):
    MANDATORY_BILLING_FIELDS = ["zipcode","name", "phone", "email", "street", "city", "country_id"]

    @route(['/my/account'], type='http', auth='user', website=True)
    def account(self, redirect=None, **post):
        self.MANDATORY_BILLING_FIELDS = ["zipcode", "name", "phone", "email", "street", "city", "country_id"]
        #self.OPTIONAL_BILLING_FIELDS = ["zipcode", "state_id", "vat", "company_name"]
        self.OPTIONAL_BILLING_FIELDS = ["zipcode", "state_id", "vat", "company_name"]
        if post.get('country_id'):
            country = request.env['res.country'].sudo().search([('id','=',post.get('country_id'))])
            if country and not country.zip_required:
    ...
    @http.route(['/shop/country_information/<model("res.country"):country>'], type='json', auth="public", methods=['POST'],
                website=True)
    def country_information(self, country, **kw):
        """
        @Author : Angel Patel
        Return zip is required or not based on the country.
        :param country:
        :param kw:
        :return:
        """
